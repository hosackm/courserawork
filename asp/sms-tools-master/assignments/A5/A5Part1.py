...
def minFreqEstErr(inputFile, f):
    ...
    fmin, Mmin, Nmin, fminerror = 100000, 100000, 100000, 10000000

    for k in range(5, 13):
        M = 100 * k + 1
        N = M+1
        while not UF.isPower2(N):
            N += 2
        ...
        mid = int(fs*0.5)
        halfm = int(M/2)
        x1 = x[mid-halfm:mid+halfm+1]

        mag, phase = DFT.dftAnal(x1, w, N)
        ploc = UF.peakDetection(mag, t)[0]
        iploc, iphase, imag = UF.peakInterp(mag, phase, ploc)
        fest = iploc * fs / N

        ferror = abs(fest-f) # abs(1 - (fest / f))

        if ferror < fminerror:
            fmin, Mmin, Nmin, fminerror = fest, M, N, ferror

    return fmin, Mmin, Nmin

if __name__ == "__main__":
   print(minFreqEstErr("/home/matt/Downloads/sms-tools-master/sounds/sine-490.wav", 490))
   print(minFreqEstErr("/home/matt/Downloads/sms-tools-master/sounds/sine-1000.wav", 1000))
   print(minFreqEstErr("/home/matt/Downloads/sms-tools-master/sounds/sine-200.wav", 200))
