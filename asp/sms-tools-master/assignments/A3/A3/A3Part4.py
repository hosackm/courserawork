...
def suppressFreqDFTmodel(x, fs, N):
    ...
    return outputScaleFactor*dftSynth(mX, pX, M), outputScaleFactor*dftSynth(mX2, pX, M)
    

if __name__ == "__main__":
    from scipy.fftpack import fft
    s1 = np.sin(2. * np.pi * 40 * np.arange(0, 1, 1/48000.))
    s2 = np.sin(2. * np.pi * 100 * np.arange(0, 1, 1/48000.))
    s3 = np.sin(2. * np.pi * 200 * np.arange(0, 1, 1/48000.))
    s4 = np.sin(2. * np.pi * 1000 * np.arange(0, 1, 1/48000.))

    before, after = suppressFreqDFTmodel(s1+s2+s3+s4, 48000, 65536)

    X1, X2 = fft(before)[:len(before)/2], fft(after)[:len(after)/2]
    plt.plot(20. * np.log(np.abs(X1)))
    plt.plot(20. * np.log(np.abs(X2)))
    plt.show()

    s1 = np.sin(2. * np.pi * 23 * np.arange(0, 1, 1/48000.))
    s2 = np.sin(2. * np.pi * 36 * np.arange(0, 1, 1/48000.))
    s3 = np.sin(2. * np.pi * 230 * np.arange(0, 1, 1/48000.))
    s4 = np.sin(2. * np.pi * 900 * np.arange(0, 1, 1/48000.))
    s5 = np.sin(2. * np.pi * 2300 * np.arange(0, 1, 1/48000.))

    before, after = suppressFreqDFTmodel(s1+s2+s3+s4+s5, 48000, 65536)
